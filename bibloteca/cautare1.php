...
if((!empty($titlu))&&(!empty($autor))&&(!empty($editura))){
	$sql="SELECT * FROM carte,editura,autor WHERE carte.TITLU LIKE '%$titlu%' AND autor.NUME LIKE '%$autor%' AND editura.NUME_ED LIKE '%$editura%'";
			...
			{	$id1=$row['ID_CARTE'];
				...
				$autor=$row['NUME'];
				$titlu=$row['TITLU'];
				echo "<tr><td>".$i."</td><td>".$autor."</td><td>".$titlu."<td>".$editura."</td></td><td><a href=\"imprumuta.php?id=$id1\"> imprumuta </a></td></tr>";
			$i++;
				}
	
}elseif((!empty($titlu))&&(empty($autor))&&(!empty($editura)))
			{
				$sql="SELECT * FROM carte,editura,autor WHERE carte.TITLU LIKE '%$titlu%' AND editura.NUME_ED LIKE '%$editura%'";
				$result=mysql_query($sql);
			$i=0;
			while($row=mysql_fetch_array($result))	
			{	$id1=$row['ID_CARTE'];
...
elseif((!empty($titlu))&&(!empty($autor))&&(empty($editura)))
			{
				$sql="SELECT * FROM carte,editura,autor WHERE carte.TITLU LIKE '%$titlu%' AND autor.NUME LIKE '%$autor%'";
				$result=mysql_query($sql);
			$i=0;
			while($row=mysql_fetch_array($result))	
			{$id1=$row['ID_CARTE'];
				...
				$autor=returnAutor($row['ID_AUTOR']);
				echo "<tr><td>".$i."</td><td>".$autor."</td><td>".$row['TITLU']."<td>".$editura."</td></td><td><a href=\"imprumuta.php?id=$id1\"> imprumuta </a></td></tr>";
			$i++;
			}	}
			elseif((empty($titlu))&&(!empty($autor))&&(!empty($editura)))
			{
				$sql="SELECT * FROM carte,editura,autor WHERE editura.NUME_ED LIKE '%$editura%' AND autor.NUME LIKE '%$autor%'";
				$result=mysql_query($sql);
			$i=0;
			while($row=mysql_fetch_array($result))	
			{$id1=$row['ID_CARTE'];
...
{		
		...
		{	$id1=$row['ID_CARTE'];
		$editura=returnEditura($row['ID_EDITURA']);
			echo "<tr><td>".$i."</td><td>".$autor_nume."</td><td>".$row['TITLU']."<td>".$editura."</td></td><td><a href=\"imprumuta.php?id=$id1\"> imprumuta </a></td></tr>";
			$i++;
			}
	}elseif(!empty($editura)&&(empty($autor))&&(empty($titlu)))
	{
		...
		$nume_editura=$row['NUME_ED'];
			...
			{$id1=$row['ID_CARTE'];
				$autor=returnAutor($row['ID_AUTOR']);
				echo "<tr><td>".$i."</td><td>".$autor."</td><td>".$row['TITLU']."</td><td>".$nume_editura."</td></td><td><a href=\"imprumuta.php?id=$id1\"> imprumuta </a></td></tr>";
				$i++;
				}
		...
		elseif(!empty($titlu)&&(empty($autor))&&(empty($editura)))
		{
			...
			{$id1=$row['ID_CARTE'];
				$editura=returnEditura($row['ID_EDITURA']);
				$autor=returnAutor($row['ID_AUTOR']);
				echo "<tr><td>".$i."</td><td>".$autor."</td><td>".$row['TITLU']."</td><td>".$editura."</td></td><td><a href=\"imprumuta.php?id=$id1\"> imprumuta </a></td></tr>";
				$i++;
				}		
			}
			if($i==0)
